...
function getVersion(body: string): number {
  const matched = body.match(/Web3[\s-]+Token[\s-]+Version: \d/);
...
export function decrypt(token: string): DecrypterResult {
  ...
  const signatureParams = fromRpcSig('0x' + signatureBuffer.toString('hex'));
