...
function getDomain(sections: MessageSections): string | undefined {
  if (
...
function getStatement(sections: MessageSections): string | undefined {
...
function parseBody(lines: string[]): DecryptedBody {
  const sections = splitSections(lines);
  const main_section = sections[sections.length - 1].join('\n');
  const parsed_body = parseAsHeaders(main_section);
  ...
  return parsed_body as unknown as DecryptedBody;
}

export function verify(token: string, opts: VerifyOpts = {}) {
